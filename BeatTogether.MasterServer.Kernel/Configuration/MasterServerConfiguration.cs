...
namespace BeatTogether.MasterServer.Kernel.Configuration
{
    public class MasterServerConfiguration
    {
        ...
        public string MasterServerVersion { get; } = "1.5.0";
        public string[] SupportedDediServerVersions { get; } = { "1.6.0" }; //for example, if 1.1 is here, then 1.1.1, 1.1.5, 1.1.23, would all be accepted verisions and 1.2.3 would not. Only change when dedi and master would be incompat otherwise
